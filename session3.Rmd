...
Before starting to write code in RStudio, we need to create an R Project.  The
idea behind an R project is  to have a workspace where you can keep all the
files and settings associated with the project together.  In that way, next
time you open the R Project it would be easier to resume work.  

We are going to create a new project in the directory of data files that you 
should already have downloaded. If you haven't got it yet, you can download it
from here:

https://bioinformatics-core-shared-training.github.io/Reverse_Summer_School_2023_PreCourse/Bioinformatics_workshop_data.zip

Then unzip the folder somewhere on your laptop.

To create an "R Project":

1. Start RStudio.
2. Under the `File` menu, click on `New Project`. Choose `Existing Directory`
3. Click the `Browse...` button and locate the **Bioinformatics_workshop_data/R_Intro**
...
5. (Optional) Set Preferences to 'Never' save workspace in RStudio:
    ...
    as this would save all the data that is loaded into R into the `.RData` file.
    To turn that off, go to `Tools` --> `Global Options` and select the 'Never'
    option for `Save workspace to .RData' on exit.`
