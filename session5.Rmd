...
patient_details_of_interest <- select(patients_of_interest, 
                                      Patient_ID, 
                                      Survival_time, 
                                      Tumour_stage, 
...
```{r eval = FALSE}
patients_of_interest <- select(patients_of_interest, 
                               Patient_ID, 
                               Survival_time, 
                               Tumour_stage, 
                               Nottingham_prognostic_index)
...
metabric <- metabric %>%
    mutate(NPI = (0.2 * Tumour_size) 
                    + Lymph_node_status + Neoplasm_histologic_grade)
...
patients_of_interest <- metabric %>%
  ...
  filter(Chemotherapy == "NO" & Radiotherapy == "NO" & Survival_time_years > 20)
